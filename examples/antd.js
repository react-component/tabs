...
function construct(start, num) {
  ...
  for (let i = start; i < start + num; i++) {
    ends.push(
      <TabPane placeholder={`loading ${i}`} tab={`tab ${i}`} disabled={!!(i % 2)} key={index}>
        <PanelContent id={i} />
      </TabPane>,
    );
    index++;
      ...
      <div style={{ margin: 20 }}>
        ...
        <div>
          <Tabs
            ...
            style={style}
            tabBarPosition={this.state.tabBarPosition}
            renderTabBar={() => <InkTabBar onTabClick={this.onTabClick} tabBarGutter={40} />}
            renderTabContent={() => <TabContent style={contentStyle} />}
            onChange={this.onChange}
          >
            {ends2}
        ...
        <div>
          ...
          <Tabs
            ...
            tabBarPosition={this.state.tabBarPosition}
            renderTabBar={() => (
              <ScrollableInkTabBar ref={this.saveBar} onTabClick={this.onTabClick} {...iconProps} />
            )}
            renderTabContent={() => <TabContent style={contentStyle} />}
            onChange={this.onChange2}
          >
